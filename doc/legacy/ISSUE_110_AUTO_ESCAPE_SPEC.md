...
**書式**: `<!-- m:autoEscape="true|false" -->` または `<!-- m:autoEscape=true|false -->`
...
レイアウトテンプレートとページテンプレートを組み合わせて使う場合、`<!-- m:autoEscape -->` は **テンプレート境界（m:insert の描画境界）ごと** に評価されます。

- ページ側に記述した directive は、ページ描画部分にのみ適用されます。
- レイアウト側の後続ノードには、ページ側 directive の設定は波及しません。
- レイアウト側に記述した directive は、レイアウト側ノードに適用されます。

ページ側に記述された directive については、共通レイアウトへ差し込まれた後の描画順で判定されます。つまり、差し込み位置に展開されたページ内容のうち、directive より後に存在するノードだけに反映されます（差し込み位置の前後にあるレイアウト側ノードへは反映されません）。

したがって、共有レイアウト利用時は「ページ設定 > グローバル設定」「レイアウト設定 > グローバル設定」がそれぞれの描画スコープ内で有効になります。

```
適用イメージ:
- ページ描画スコープ: ページ設定 > （呼び出し元から引き継いだ設定） > グローバル設定
- レイアウト描画スコープ: レイアウト設定 > グローバル設定
```
