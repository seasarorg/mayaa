...
5. 既にエスケープ済み値の自動検出（`escapeDetectionLevel`）は誤検出/検出漏れの可能性がある
...
2. `escapeDetectionLevel` を `strict` にして判定条件を厳格化する
3. 問題ページだけ `<!-- m:autoEscape="false" -->` で一時的に無効化する
...
**原因:**
- `escapeDetectionLevel=normal` は「一部のパターン一致」で判定するため、独自エスケープ規則と相性が悪い場合がある

**対応:**
1. `escapeDetectionLevel=strict` を試す
2. 独自エスケープ util の戻り値を `${=...}` で明示的に出力する
3. 判定に依存しないよう、エスケープ実装の統一ルールを決める
