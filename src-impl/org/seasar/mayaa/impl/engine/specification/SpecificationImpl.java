...
public class SpecificationImpl extends ParameterAwareImpl implements Specification {
    ...
    /**
     * ID injection 索引キャッシュ（この spec の .mayaa のみ）。
     * {@code deprecate()} 呼び出し時にクリアされ、次のビルド時に再構築される。
     * volatile 書き込みで可視性を保証する。並行して複数スレッドが同時に構築しても
     * 結果は等価なので、最後の書き込みが残るだけで問題ない。
     */
...
//    }
    ...
    /**
     * この spec を起点とした ID injection 索引マップを返す。
     * まだ構築されていない場合は {@code null} を返す。
     */
    public Map<String, SpecificationNode> getCachedIdNodeMap() {
        return _idNodeMap;
    }

    /**
     * ID injection 索引マップをセットする。
